    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <div className={`bg-gradient-to-br ${post.image} h-56 md:h-72 flex items-center justify-center relative`}>
        ...
        <div className="relative z-10 text-center px-4">
          <span
            className={`inline-flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-full mb-4 ${categoryColors[post.category]}`}
          >
              ...
              <div className="space-y-4">
                {relatedPosts.map((related) => (
                  <Link key={related.slug} href={`/blog/${related.slug}`} className="group block">
                    <div
                      className={`bg-gradient-to-br ${related.image} h-20 rounded-lg flex items-center justify-center mb-2`}
                    >
                      <BookOpen className="w-6 h-6 text-white opacity-40" />
                    </div>
                    <span
                      className={`text-xs font-semibold px-2 py-0.5 rounded-full ${categoryColors[related.category]}`}
                    >
                      {related.category}
                    </span>
